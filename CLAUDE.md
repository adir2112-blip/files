...
- חוסם בפועל קריאות `fetch` לכל URL שמכיל `/functions/v1/` (Edge Functions של Supabase —
  ...
  עובדות רגיל.
  **חריג**: `/functions/v1/agent-login` לא נחסם — היא לא שולחת שום דבר החוצה (רק מנפיקה
  session לנציג לפי PIN, בדיוק כמו ב-production), כך שאפשר לבדוק כניסת נציגים גם בדמה.
  אם מוסיפים בעתיד Edge Function חדשה שגם היא לא שולחת החוצה, יש להוסיף לה חריג דומה.
